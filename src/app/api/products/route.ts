...
export async function GET(req: NextRequest) {
  try {
    ...
    const dbProducts = await prisma.product.findMany({
      ...
      include: {
        // IMPORTANT: this must match the Prisma schema relation name
        Listing: true,
      },
      skip,
      take: perPage,
    ...
    if (sort === "price-asc") {
      sorted.sort(
        (a: any, b: any) =>
          getBestPrice({ listings: a.Listing ?? [] }) -
          getBestPrice({ listings: b.Listing ?? [] })
      );
    ...
    const products: ProductResponse[] = sorted.map((p: any) => ({
      ...
      // Map Prisma's `Listing` relation to the public `listings` field
